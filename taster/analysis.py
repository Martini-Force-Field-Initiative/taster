...
warnings.showwarning = (
    lambda message, category, filename, lineno, file=None, line=None: _logger.warning(
...
def _save_diagnostics(diagnostics_dir, estimator_name, data_list, result):
    ...
    if estimator_name == "MBAR":
        ...
        ax.figure.savefig(
            diagnostics_dir / "mbar_overlap_matrix.png", dpi=300, bbox_inches="tight"
        )
        plt.close(ax.figure)
    else:
...
def TIRoutine(
    workingdir, T=298, cutoff=5000, states=None, estimator="MBAR", diagnostics_dir=None
):
    """
    ...
    if estimator == "both":
        ...
        raise ValueError(
            f"Unknown estimator '{estimator}'. Choose 'TI', 'MBAR', or 'both'."
        )
...
def _format_report(df, resname):
    ...
    for rep, group in df[df["rep"] != "avg"].groupby("rep"):
        ...
        for _, row in group.iterrows():
            lines.append(
                f"  {row['solvent']}/water: {row['logP']:.4f} +- {row['logP_err']:.4f} LogP units\n"
            )
            lines.append(
                f"  {row['solvent']}/water: {row['dG']:.4f} +- {row['dG_err']:.4f} kJ/mol\n"
            )
