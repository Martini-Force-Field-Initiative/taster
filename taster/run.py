...
def _run_ti_state(
    ...
    """
    if fep_min_mdp is None:
        fep_min_mdp = files("taster.data.mdps") / "template-CG_minimization-fep.mdp"
    if fep_rel_mdp is None:
        fep_rel_mdp = files("taster.data.mdps") / "template-CG_relaxation-fep.mdp"
    if fep_prod_mdp is None:
        fep_prod_mdp = files("taster.data.mdps") / "template-CG_fep-vdw.mdp"
    ...
    _replace_words_in_file(
        fep_rel_mdp,
        state_dir / "rel.mdp",
        ["MOL", "INIT-LAMBDA-STATE", "TEMPERATURE"],
        [resname, str(state), str(T)],
    )
    _replace_words_in_file(
        fep_prod_mdp,
        state_dir / "fep.mdp",
        ["MOL", "INIT-LAMBDA-STATE", "TEMPERATURE", "NSTEPS"],
        [resname, str(state), str(T), str(nsteps)],
    )
...
def _tracked_ti_state(
    resname, state, workingdir, offset, gmx, sem, offset_pool, T=298, nsteps=1250000
):
    """
    ...
    try:
        _run_ti_state(
            resname, state, workingdir, offset=offset, gmx=gmx, T=T, nsteps=nsteps
        )
    finally:
...
def run_partitions(
    ...
    def _reap(pending):
        ...
        for proc, rep, solvent, state in pending:
            ...
            if proc.exitcode != 0:
                log_path = (
                    output_dir / resname / str(rep) / solvent / str(state) / "log.out"
                )
    ...
    if failures:
        ...
        for rep, solvent, state, log_path in failures:
            lines.append(
                f"  rep {rep}, solvent '{solvent}', state {state} (see {log_path})"
            )
        raise RuntimeError("\n".join(lines))
